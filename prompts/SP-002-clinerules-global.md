...
| feature/{Timebox}/{IDEA-NNN}-{slug} | Single feature or fix grouped by timebox. Timebox: `YYYY-QN` (Quarter) or `Sprint-NN`. | Branch from develop or develop-vX.Y, merge back via PR, then delete. Examples: `feature/2026-Q2/IDEA-101-authentication`, `feature/Sprint-42/IDEA-101-authentication` |
...
| release/vX.Y.Z | Release stabilization buffer. Isolates release polish from ongoing development. | Branch from develop when release is ready; all release fixes land here; merged to main and develop when stable. |
...
1. Branch from develop (ad-hoc) or develop-vX.Y (scoped) -> `feature/{Timebox}/{IDEA-NNN}-{slug}`
...
When a release branch (`release/vX.Y.Z`) is active, development for the next release **continues on develop in parallel**. This allows the team to:
- Polish the current release on `release/vX.Y.Z`
...
The Orchestrator is the authoritative default mode. All mode switches should be
driven by the Orchestrator. Non-Orchestrator modes must NOT assume they are the entry point.
